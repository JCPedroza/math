...
class Application(Frame):
    ...
    def initUI(self):
        ...
        abcfactorsButton = Button(self, text="abc factors", command=self.abc_factors_of)
        abcfactorsButton.grid(row=3, column=0)
    ...
    def abc_factors_of(self):
        """ Return factors of ac that together sum b. """
        entry = self.get_entry()
        if self.input_is_valid(entry, 3):
            result = Math.abc_factors_of(*entry)
            self.result.config(text=result)
